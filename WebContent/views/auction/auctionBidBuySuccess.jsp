...
<body>
	...
	<%
		...
		int sellerBirthYear = Integer.parseInt(seller.getBirth().split("/")[0]);
		Calendar cal = Calendar.getInstance();
		int year = cal.get(Calendar.YEAR);

		int sellerAge = (year - sellerBirthYear) + 1;
		String sellerGender = "";
		if (seller.getGender() == 'M') {
			sellerGender = "남";
		} else {
			sellerGender = "여";
		}

		int buyerBirthYear = Integer.parseInt(buyer.getBirth().split("/")[0]);

		int buyerAge = (year - buyerBirthYear) + 1;
		String buyerGender = "";
		if (buyer.getGender() == 'M') {
			buyerGender = "남";
		} else {
			buyerGender = "여";
		}
							...
							<div class="row">
								...
								<div class="col-9 col-md-8 col-lg-9">
									<div>
										...
										<span style="display:block" class="infoFont my-2">판매자 닉네임 : <%=seller.getUserNick()%></span><br>
										<span style="display:block" class="infoFont my-2">판매자 게시물 수 : <%=postCount%> 개</span><br>
										<span style="display:block" class="infoFont my-2">판매자 신고 횟수 : <%=seller.getReported()%>번</span><br>
										<span style="display:block" class="infoFont my-2">판매자 핸드폰 번호 : <%=seller.getPhone()%></span><br>
										<span style="display:block" class="infoFont my-2">판매자 나이(성별) : <%=sellerAge%>세(<%=sellerGender%>)</span><br>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="col-6 col-md-6">
						...
						<div class="selleInformation">
							<div class="row">
								...
								<div class="col-9 col-md-8 col-lg-9">
									<div>
										<span style="display:block" class="infoFont my-2">구매자 아이디 : <%=buyer.getUserId()%></span> <br>
										<span style="display:block" class="infoFont my-2">구매자 닉네임 : <%=buyer.getUserNick()%></span><br>
										<span style="display:block" class="infoFont my-2">구매자 신고 횟수 : <%=buyer.getReported()%>번</span><br>
										<span style="display:block" class="infoFont my-2">구매자 핸드폰 번호 : <%=buyer.getPhone()%></span><br>
										<span style="display:block" class="infoFont my-2">구매자 나이(성별) : <%=buyerAge%>세(<%=buyerGender%>)</span><br>
										<span style="display:block" class="infoFont my-2">구매자 주소 : <%=buyer.getAddress()%></span><br>
									</div>
								</div>
							</div>
						</div>
	...
	$(function() {
		$("#buyCancle").click(function() {
			if(confirm('구매를 취소하시겠습니까?')) {
				location.href="/buyCancel.do?postNo=<%=p.getPostNo()%>&userId=<%=buyer.getUserId()%>";
			} else {
				return false;
			}
		});
            ...
            if (rsp.success) {
                ...
                alert(msg);
  				$.ajax({
            		url: "/sendSmsTrade.do",
            		data: {
            			receiver: "<%=seller.getPhone()%>"
            		},
            		type: "post"
            	});		

                location.href="/trade.do?postNo=<%=p.getPostNo()%>&buyerId=<%=buyer.getUserId()%>&sellerId=<%=p.getWriter()%>";
            } else {
                var msg = '결제에 실패하였습니다.';
