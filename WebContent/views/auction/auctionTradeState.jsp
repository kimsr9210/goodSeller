...
<body>
	...
	<div id="wrap">
		...
		<div id="contents">
			<div class="container">
				...
				<hr>
				<span class="infoFont">거래 상태</span>
				<div id="contents-deail-4" class="row">
							...
							<div class="row">
								<div class="col-12">
									...
									<%} else { %>
										...
										<span class="infoFont">물품 수령 후 구매 확정 버튼을 눌러 거래를 종료하여 주십시오</span><br>
									<%} %>
									</div>
								</div>
							</div>
				...
				<div class ="row text-center my-3">
					...
					<%if (m.getUserId().equals(p.getWriter())) {
					%>
						<button type="button" class="btn btn-dark" id="sellerEndBtn">거래 종료</button>
					<%} else { %>
						<button type="button" class="btn btn-dark" id="buyerEndBtn">구매 확정</button>
					<%} %>
					</div>
				</div>
				<br>
	...
	<script>
	$(function() {
		$("#sellerEndBtn").click(function() {
			if(confirm('거래를 종료하시겠습니까? 종료하시면 되돌릴 수 없습니다.')) {
				location.href="/tradeSellerEnd.do?postNo=<%=p.getPostNo()%>&sellerId=<%=seller.getUserId()%>&buyerId=<%=buyer.getUserId()%>";
			} else {
				return false;
			}
		});
		
		$("#buyerEndBtn").click(function() {
			if(confirm('거래를 종료하시겠습니까? 종료하시면 되돌릴 수 없습니다.')) {
				location.href="/tradeBuyerEnd.do?postNo=<%=p.getPostNo()%>&sellerId=<%=seller.getUserId()%>&buyerId=<%=buyer.getUserId()%>";
			} else {
				return false;
			}
		});
